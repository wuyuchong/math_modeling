...
wide_window.plot('SO2监测浓度(μg/m³)', './output/series_base_line.png', model=baseline)
# ----------------------------------------------------------------
...
#  print('Input shape:', wide_window.example[0].shape)
#  print('Output shape:', baseline(wide_window.example[0]).shape)
wide_window.plot('SO2监测浓度(μg/m³)', './output/series_linear.png', model=linear)
...
plt.savefig('./output/importence_linear.png')
# ----------------------------------------------------------------
...
conv_window = WindowGenerator(
    ...
    train_df=train_df, val_df=val_df, test_df=test_df,
    label_columns=['SO2监测浓度(μg/m³)'])
conv_window.plot('SO2监测浓度(μg/m³)', './output/conv_window.png')
...
ax1.plot(ts_out)
ax2.plot(ts_test)
plt.savefig('./output/ts_out_test.png')
